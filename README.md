# Ameliq Family — Система управления семейными делами

**Ameliq Family** — это персональный веб-сервис для ведения домашнего хозяйства, учета финансов и планирования задач. Построен на современном стеке технологий Laravel 12, Inertia.js и React.

## 🚀 Основной функционал

### 📊 Дашборд (Центр управления)
Визуализация состояния дел в реальном времени:
- **Финансовый период:** Уникальная логика учета — отчетный месяц начинается **10-го числа** (удобно для привязки к датам выплат).
- **Графики баланса:** История доходов, расходов и чистого остатка за последние 6 месяцев.
- **Расходы по категориям:** Круговая диаграмма с цветовой индикацией трат.
- **Календарь задач:** Наглядное отображение загруженности по дням.

### 💰 Финансы
- **Учет доходов:** Фиксация всех поступлений в семейный бюджет.
- **Учет расходов:** Детальная запись трат с привязкой к категориям.
- **Категории:** Полностью настраиваемый справочник категорий (продукты, авто, ЖКХ и т.д.) с выбором цвета для графиков.
- **Авторасходы (Automation):** Система регулярных платежей. Вы задаете день месяца, и сервис автоматически создает запись о расходе (например, за интернет или ипотеку).

### ✅ Задачи (To-Do)
- Управление семейными делами и поручениями.
- Установка дедлайнов (Due Date) и точного времени выполнения.
- Отслеживание статуса выполнения прямо из общего списка или календаря.
- **Telegram-уведомления:** Ежедневная рассылка общего списка семейных задач на текущий день в 08:00.

### 🛒 Списки
- Создание нескольких списков (например, «Продукты», «Хозтовары»).
- Быстрое добавление, редактирование и вычеркивание элементов.

### 📝 Заметки
- Хранение важной информации и паролей.
- **Безопасность:** Пароли хранятся в зашифрованном виде (AES-256).
- **ПИН-защита:** Для просмотра или копирования пароля требуется 4-значный код.
- **Поиск:** Быстрый поиск по названию заметки.
- **Избранное:** Возможность закреплять важные заметки сверху.

### 🤖 Telegram Ассистент
Система автоматических уведомлений для всей семьи:
- **Утренний дайджест:** Каждый день в 08:00 бот присылает список всех невыполненных задач на сегодня.
- **Детализация:** Сообщение включает название задачи, время выполнения и комментарий (если есть).
- **Общий доступ:** Неважно, кто создал задачу — её увидят все участники семьи с подключенным Telegram.

## 🛠 Технологический стек

- **Backend:** PHP 8.2+, [Laravel 12](https://laravel.com)
- **Frontend:** [React](https://reactjs.org) + [Inertia.js](https://inertiajs.com)
- **Styling:** [Tailwind CSS](https://tailwindcss.com)
- **Build Tool:** [Vite](https://vitejs.dev)
- **Database:** MySQL / PostgreSQL / SQLite

## ⚙️ Настройка автоматизации

Для работы авторасходов и Telegram-уведомлений необходимо настроить планировщик Laravel.

1. **Добавьте запись в crontab:**
   ```cron
   * * * * * cd /путь/к/проекту && php artisan schedule:run >> /dev/null 2>&1
   ```
   *Планировщик запускает:*
   - *Авторасходы: ежедневно в 06:00.*
   - *Telegram-дайджест: ежедневно в 08:00.*

2. **Настройка Telegram бота:**
   - Создайте бота в [@BotFather](https://t.me/BotFather) и получите токен.
   - Добавьте в `.env`:
     ```env
     TELEGRAM_BOT_TOKEN=ваш_токен
     ```
   - Узнайте свой Chat ID (например, через [@userinfobot](https://t.me/userinfobot)).
   - Привяжите ID к пользователю в базе данных через Tinker:
     ```bash
     php artisan tinker --execute="\App\Models\User::where('email', 'admin@example.com')->update(['telegram_chat_id' => 'ВАШ_ID']);"
     ```

3. **Ручной запуск команд:**
   - Проверка авторасходов: `php artisan auto-expenses:apply`
   - Тест рассылки задач: `php artisan tasks:send-daily`

## 📦 Установка и запуск

1. Склонируйте репозиторий.
2. Установите зависимости:
   ```bash
   composer install
   npm install
   ```
3. Настройте окружение:
   ```bash
   cp .env.example .env
   php artisan key:generate
   ```
4. Выполните миграции:
   ```bash
   php artisan migrate
   ```
5. Соберите фронтенд:
   ```bash
   npm run build # или npm run dev для разработки
   ```

---
*Разработано для семьи Ameliq.*
